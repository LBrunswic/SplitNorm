...
if TEST:
    ...
    images = (images/normalization).astype('float32').reshape(-1,32,32,3)
...
channeller_archs = [
    [
        ConvolutionalKernel.ChannellerConstructors.channeller_sequential_resnet,
        {
...
picture_coord = np.mgrid[[slice(a, b, e) for a, b, e in limits]].transpose().astype('float32').reshape(1,xmax, ymax,2)
...
pictures_coord = np.concatenate([np.broadcast_to(picture_coord,(dataset_size,xmax, ymax,2)), images],axis=-1)
pictures_coord = pictures_coord.reshape((-1, 32*5, 32))

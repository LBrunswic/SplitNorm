...
class CommandedTransformedDistribution(tf.keras.Model):
    ...
    def score(self,sample_batch,command_batch):
        return self._score(self.parametrized_transformed_distribution,sample_batch,command_batch)

    def prob(self,sample_batch,command_batch):
        return self._prob(self.parametrized_transformed_distribution,sample_batch,command_batch)

    def sample(self,command_batch):
        return self._sample(self.parametrized_transformed_distribution,command_batch)
...
class FlowEnsemble(tf.keras.Model):
    ...
    def build_dist(self,base_distribution,*build_args,**build_kwargs):
        ...
        def _score(transformed_distribution,sample_batch,command_batch):
            print(transformed_distribution.__repr__)
            return -transformed_distribution.log_prob(
                sample_batch,
                bijector_kwargs =  {'command': command_batch},
            )

        def _prob(transformed_distribution,sample_batch,command_batch,*args,**kwargs):
        ...
        def _sample(transformed_distribution,command_batch,*args,**kwargs):
        ...
        return CommandedTransformedDistribution(
            tfd.TransformedDistribution(distribution=base_distribution, bijector=self.flow_family, *build_args, **build_kwargs),
            _score,
            _prob,
            _sample,
...
class ConvKernel(tf.keras.Model):
    def __init__(self, kernel_ensemble, channeller, commander, channel_dim=0, command_dim=0,name='ConvFlow'):
    ...
    def build_dist(self,base_distribution):
        ...
        def _score(transformed_distribution,sample_batch,command_batch,*args,**kwargs):
            weighted_channel_batch = self.channeller((sample_batch, command_batch))
            # (data_batch_size,command_batch_size,command_dim+1)
            A = [
                self.reshape(transformed_distribution.score(sample_batch,  self.commander((channel_batch, command_batch)), *args,**kwargs))
        ...
        def _sample(transformed_distribution,channel_command_batch,*args,**kwargs):
            samples = transformed_distribution.sample(
